...
- [x] **UI-03**: Clicking any agent tile replaces the Explorer workspace folders with only that agent's worktree folder, so the file tree shows exclusively the selected agent's working directory
- [x] **UI-04**: Clicking an agent tile switches the full VS Code context -- Explorer shows only the agent's worktree, code editor opens a file from the worktree, and the agent CLI terminal is focused
